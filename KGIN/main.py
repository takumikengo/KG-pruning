...
if __name__ == '__main__':
    ...
    wandb.init(
        project="KGIN-recommender",
        config=vars(args),  # 全てのハイパーパラメータを保存
        name=f"run_{args.dataset}_{time()}" # 実行ごとに名前を分けると管理しやすいです
    )
    seed = args.seed
    ...
    for epoch in range(args.epoch):
        ...
        if epoch % 10 == 9 or epoch == 1:
            ...
            cur_best_pre_0, stopping_step, should_stop = early_stopping(ret['recall'][2], cur_best_pre_0,
            ...
            """save weight"""
            if ret['recall'][2] == cur_best_pre_0 and args.save:
                torch.save(model.state_dict(), args.out_dir + 'model_' + args.dataset + "_" + args.kg_file + '.ckpt')
